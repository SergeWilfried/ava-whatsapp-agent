"""
Setup MongoDB indexes for optimized performance
...
async def setup_indexes():
    ...
    if not db_name:
        print("❌ MONGODB_DB_NAME not set in .env file")
    ...
    try:
        ...
        # This is the most important index for the optimized webhook handler
        ...
        # Your Business model already has: { subDomain: 1 } (unique)
        # We need a compound one with isActive for faster queries
        print("\n2. Checking subdomain_lookup_idx...")
        subdomain_exists = await check_index_exists(businesses, "subDomain_1")

        if subdomain_exists:
            print("   ⏭️  subdomain index already exists (subDomain_1)")
            print("   ℹ️  Note: Existing index is sufficient for queries")
        else:
            if await create_index_if_missing(
                businesses,
        ...
        for idx in indexes:
            index_name = idx['name']
            is_new = "🆕" if index_name in ["webhook_lookup_idx", "subdomain_lookup_idx", "active_whatsapp_businesses_idx"] else ""
        ...
        if whatsapp_count == 0:
            print("\n⚠️  Warning: No businesses found with WhatsApp enabled!")
        ...
        print("\n" + "="*60)
        if created_count > 0:
            print(f"✅ Index setup completed! Created {created_count} new index(es)")
        else:
            print("✅ All required indexes already exist!")
        print("="*60)

        print("\nNext steps:")
        print("1. ✅ Webhook handler already updated to use optimized service")
